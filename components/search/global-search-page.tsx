...
function getBannerStatusText(
  ...
  if (isSearching) {
    if (mode === "image") return "Ищем похожие вещи...";
    return `Ищем по запросу "${query}"...`;
  }
  if (mode === "image") {
    return totalCount > 0 ? `Найдено ${totalCount} результатов по фото` : "Похожих вещей не найдено";
  }
  return totalCount > 0 ? `Найдено ${totalCount} результатов` : `Ничего не найдено по запросу "${query}"`;
}
...
function NotFoundPrompt({ activeSession }: { activeSession: ActiveSearchSession | null }) {
...
function usePhotoSearchCapture(
  setActiveSession: React.Dispatch<React.SetStateAction<ActiveSearchSession | null>>,
  setSearchResults: React.Dispatch<React.SetStateAction<SearchHit[]>>,
  ...
  return useCallback(
    ...
    [clearSearchSession, setActiveSession, setIsSearching, setSearchQuery, setSearchResults, setTotalCount]
  );
}
...
function SearchInputBlock({
  ...
  searchQuery,
  setSearchQuery,
  setIsPhotoSearchOpen,
  isSearching,
  clearSearchSession,
}: {
  isSearchActive: boolean;
  isPhotoSearchActive: boolean;
  searchQuery: string;
  setSearchQuery: (val: string) => void;
  setIsPhotoSearchOpen: (val: boolean) => void;
  isSearching: boolean;
  clearSearchSession: () => void;
}) {
  return (
...
function useTextSearch(
  ...
  return useCallback(
    ...
    [setActiveSession, setIsSearching, setSearchResults, setTotalCount, searchRequestIdRef, isMountedRef]
  );
}
...
export function GlobalSearchPage() {
  ...
  const performTextSearch = useTextSearch(
    setActiveSession,
    setSearchResults,
    setTotalCount,
    setIsSearching,
  ...
  const handlePhotoSearchCapture = usePhotoSearchCapture(
    setActiveSession,
    setSearchResults,
    setTotalCount,
    setIsSearching,
  ...
  const bannerStatusText = getBannerStatusText(isSearching, effectiveMode, totalCount, searchQuery.trim());
  ...
  return (
    <div className={cn("flex flex-col w-full mx-auto transition-all duration-500", isSearchActive ? "gap-4" : "gap-8 mt-[10vh] max-w-3xl")}>
      {!isSearchActive && <SearchHero />}

      {isSearchActive && <PageHeader title="Поиск" />}

      <SearchInputBlock
        isSearchActive={isSearchActive}
        isPhotoSearchActive={isPhotoSearchActive}
        searchQuery={searchQuery}
        setSearchQuery={setSearchQuery}
        setIsPhotoSearchOpen={setIsPhotoSearchOpen}
        isSearching={isSearching}
        clearSearchSession={clearSearchSession}
      />
      ...
      {isSearchActive && effectiveMode ? (
        <div className="flex flex-col gap-8 animate-in fade-in duration-300">
          ...
          {effectiveMode === "image" && !isSearching && (
            <NotFoundPrompt activeSession={activeSession} />
          )}
        </div>
      ) : (
