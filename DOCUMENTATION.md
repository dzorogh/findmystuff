...
**Важное правило**: Во время загрузки данных в окнах перемещений (и других формах с выбором помещений/мест/контейнеров) **всегда** показывать скелетон вместо сообщения "не найдены". Сообщение "не найдены" должно отображаться только после завершения загрузки, если список действительно пуст. Это предотвращает показ ошибочного сообщения "Помещения не найдены" во время загрузки данных.
