...
interface UseEntityActionsOptions {
  entityType: "containers" | "items" | "places" | "rooms" | "buildings" | "furniture";
  entityId: number;
  entityName: string;
  onSuccess?: () => void;
}
...
export const useEntityActions = ({
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await softDeleteApiClient.softDelete(entityType, entityId);
      if (response.error) throw new Error(response.error);
      toast.success(`${entityName} успешно удален`);
      onSuccess?.();
    } catch (err) {
      logError(`Ошибка при удалении ${entityName}:`, err);
  ...
  const handleRestore = async () => {
    setIsRestoring(true);
    try {
      const response = await softDeleteApiClient.restoreDeleted(entityType, entityId);
      if (response.error) throw new Error(response.error);
      toast.success(`${entityName} успешно восстановлен`);
      onSuccess?.();
    } catch (err) {
      logError(`Ошибка при восстановлении ${entityName}:`, err);
