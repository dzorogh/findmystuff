drop materialized view if exists public.mv_item_last_transition;
drop materialized view if exists public.mv_container_last_transition;
drop materialized view if exists public.mv_place_last_room_transition;

create materialized view public.mv_item_last_transition as
...
create unique index if not exists mv_item_last_transition_item_id_idx
  on public.mv_item_last_transition (item_id);

create materialized view public.mv_container_last_transition as
...
create unique index if not exists mv_container_last_transition_container_id_idx
  on public.mv_container_last_transition (container_id);

create materialized view public.mv_place_last_room_transition as
select distinct on (t.place_id)
  t.place_id,
  t.destination_id as room_id,
  t.created_at
from transitions t
where t.place_id is not null and t.destination_type = 'room'
