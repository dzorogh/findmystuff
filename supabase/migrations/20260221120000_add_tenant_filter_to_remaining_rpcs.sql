-- Добавить filter_tenant_id в RPC списков rooms, places, containers, items, furniture

-- get_rooms_with_counts
drop function if exists public.get_rooms_with_counts(text, boolean, integer, integer, text, text, boolean, boolean, boolean, bigint);
...
drop function if exists public.get_places_with_room(text, boolean, integer, integer, text, text, bigint, bigint, bigint);
...
with place_last_furniture as (
  select distinct on (t.place_id)
    t.place_id,
    t.destination_id as furniture_id
  from transitions t
...
drop function if exists public.get_containers_with_location(text, boolean, integer, integer, text, text, bigint, boolean, text);
...
-- get_items_with_room (удалить старую перегрузку с place_id, container_id)
drop function if exists public.get_items_with_room(text, boolean, integer, integer, text, bigint, bigint, bigint, boolean, text, text);
drop function if exists public.get_items_with_room(text, boolean, integer, integer, text, bigint, boolean, text, text);
...
-- get_furniture_with_counts
drop function if exists public.get_furniture_with_counts(text, boolean, integer, integer, text, text, bigint);
...
from paged p;
$$;
grant execute on function public.get_furniture_with_counts(text, boolean, integer, integer, text, text, bigint, bigint) to anon, authenticated;
