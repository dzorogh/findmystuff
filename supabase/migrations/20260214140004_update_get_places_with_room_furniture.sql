-- Добавить filter_furniture_id и furniture_id, furniture_name в get_places_with_room
-- Удалить обе перегрузки: 6-param (без фильтров) и 8-param (без furniture_id)
drop function if exists public.get_places_with_room(text, boolean, integer, integer, text, text);
drop function if exists public.get_places_with_room(text, boolean, integer, integer, text, text, bigint, bigint);
...
grant execute on function public.get_places_with_room(text, boolean, integer, integer, text, text, bigint, bigint, bigint) to anon, authenticated;
