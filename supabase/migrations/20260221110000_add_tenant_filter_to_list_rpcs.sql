-- Добавить filter_tenant_id в RPC списков для фильтрации по активному тенанту

-- get_buildings_with_counts
drop function if exists public.get_buildings_with_counts(text, boolean, integer, integer, text, text);
...
with filtered_buildings as (
  ...
  where
    (filter_tenant_id is null or b.tenant_id = filter_tenant_id)
    and (
      case
...
grant execute on function public.get_buildings_with_counts(text, boolean, integer, integer, text, text, bigint) to anon, authenticated;
