-- Доступ ко всем сущностям через RLS: MVs недоступны authenticated и не поддерживают RLS.
-- Заменяем использование MVs на SECURITY INVOKER представления, читающие из transitions
-- (и furniture) — к ним применяется RLS. RPC остаются SECURITY INVOKER.

-- 1) Представления (логика как у MVs, источник — transitions с RLS)
drop view if exists public.v_place_last_room_transition;
drop view if exists public.v_container_last_transition;
drop view if exists public.v_item_last_transition;

create view public.v_item_last_transition
...
create view public.v_container_last_transition
with (security_invoker = true)
as
select distinct on (t.container_id)
  t.container_id,
  t.destination_type,
  t.destination_id,
  t.created_at
...
order by t.container_id, t.created_at desc;

create view public.v_place_last_room_transition
with (security_invoker = true)
as
...
grant select on public.v_item_last_transition to authenticated;
grant select on public.v_container_last_transition to authenticated;
grant select on public.v_place_last_room_transition to authenticated;

-- 2) fn_item_resolved_room: читать из v_*, SECURITY INVOKER (RLS при вызове из RPC)
...
with item_transition as (
  ...
  from public.v_item_last_transition it
),
place_room as (
  select it.item_id, pr.room_id
  from item_transition it
  join public.v_place_last_room_transition pr
...
container_place_room as (
  ...
  left join public.v_place_last_room_transition pr
...
-- 3) RPC списков: переключаем на v_* (вместо mv_*), оставляем SECURITY INVOKER
...
places_count as (
  ...
  from public.v_place_last_room_transition
  where room_id is not null
  group by room_id
),
containers_count as (
  select destination_id as room_id, count(*)::bigint as containers_count
  from public.v_container_last_transition
...
filtered_places as (
  ...
  left join public.v_place_last_room_transition pr on pr.place_id = p.id
...
items_count as (
  ...
  from public.v_item_last_transition where destination_type = 'place' group by destination_id
),
containers_count as (
  select destination_id as place_id, count(*)::bigint as containers_count
  from public.v_container_last_transition where destination_type = 'place' group by destination_id
)
...
begin
  ...
  with_transition as (
    ...
    left join public.v_container_last_transition ct on ct.container_id = f.id
  ...
  items_count_all as (
    ...
    from public.v_item_last_transition t
...
item_transition as (
  ...
  left join public.v_item_last_transition it on it.item_id = f.id
),
place_room_transition as (
  select it.id, pr.room_id
  from item_transition it
  left join public.v_place_last_room_transition pr on it.destination_type = 'place' and pr.place_id = it.destination_id
),
container_terminal as (
  ...
  left join lateral (
    with recursive chain as (
      select t.destination_type, t.destination_id, 1 as depth from public.v_container_last_transition t where t.container_id = it.destination_id
      union all
      select t2.destination_type, t2.destination_id, c.depth + 1 from chain c
      join public.v_container_last_transition t2 on t2.container_id = c.destination_id
      where c.destination_type = 'container' and c.depth < 10
    )
...
container_place_room_transition as (
  ...
  from container_terminal ct
  left join public.v_place_last_room_transition pr on ct.terminal_type = 'place' and pr.place_id = ct.terminal_id
),
resolved_room as (
