...
create or replace function public.get_containers_with_location(
  ...
  p_furniture_id bigint default null
)
returns table (
...
begin
  ...
  with_transition as (
    ...
    where (p_destination_type is null or p_destination_type = 'all' or p_destination_type = '' or ct.destination_type = p_destination_type)
      and (p_furniture_id is null or (ct.destination_type = 'furniture' and ct.destination_id = p_furniture_id))
  ),
  items_count_all as (
  ...
  select p.id::bigint, p.name::text, p.entity_type_id::bigint, p.entity_type_name::text,
    ...
    (case when ct.destination_type = 'room' then r.name when ct.destination_type = 'place' then pl.name when ct.destination_type = 'container' then c2.name when ct.destination_type = 'furniture' then f.name else null end)::text as destination_name,
    ...
    (case when ct.destination_type = 'room' then ct.destination_id when ct.destination_type = 'furniture' then f.room_id when ct.destination_type = 'place' then plr.room_id else null end)::bigint as room_id,
    (case when ct.destination_type = 'room' then r.name when ct.destination_type = 'furniture' then room_f.name when ct.destination_type = 'place' then room_pl.name else null end)::text as room_name
  ...
  left join furniture f on f.id = ct.destination_id and f.deleted_at is null and ct.destination_type = 'furniture'
  left join rooms room_f on room_f.id = f.room_id and room_f.deleted_at is null and ct.destination_type = 'furniture'
  left join public.mv_place_last_room_transition plr on plr.place_id = ct.destination_id and ct.destination_type = 'place'
  left join rooms room_pl on room_pl.id = plr.room_id and room_pl.deleted_at is null and ct.destination_type = 'place'
