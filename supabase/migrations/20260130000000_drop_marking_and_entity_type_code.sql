-- Удаление полей маркировки и кода типа сущности
alter table if exists public.containers drop column if exists marking_number;
alter table if exists public.places drop column if exists marking_number;
alter table if exists public.rooms drop column if exists marking_number;
alter table if exists public.entity_types drop column if exists code;

-- Пересоздание функций списков без marking_number и entity_type_code
