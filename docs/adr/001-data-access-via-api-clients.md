...
- **Исключение для восстановления пароля:** на странице `app/(auth)/auth/forgot-password/page.tsx` разрешён прямой импорт `createClient` из `@supabase/supabase-js` (а не из `@/lib/shared/supabase/client`) с особыми опциями Auth (implicit flow, без сессии) для отправки письма сброса пароля. Это единственное место, где используется клиент с такой конфигурацией; при рефакторинге auth не заменять его на общий клиент без учёта этого сценария.
